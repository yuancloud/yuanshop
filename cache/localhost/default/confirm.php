...
<head>
	...
	<link rel="stylesheet" href="http://localhost:32185/themes/default/__RESOURCE__/recouse/css/confirm.css?v=111" />
...
.has-js .label_radio { 
	...
	background: url(http://localhost:32185/themes/default/__RESOURCE__/recouse/images/radio_none.png) no-repeat;
	background-size: 16px 16px;
}
.has-js label.r_on { 
	background-position: 0 0px;
	background: url(http://localhost:32185/themes/default/__RESOURCE__/recouse/images/radio_check.png) no-repeat;
	background-size: 16px 16px;
}
...
<input type="hidden" name="address" value="<?php echo $defaultAddress['id'];?>" />
 ...
 <ul class="payStyle">
 	...
 	<?php $ischecked=true; if(is_array($dispatch)) { foreach($dispatch as $d) { ?>
  				
                                   <li><label><input name='dispatch'  onchange='oncheckboxchange("<?php echo $issendfree==1?0:$d['price']?>");' type="radio"  value="<?php echo $d['id'];?>" <?php echo $ischecked?'checked="checked"':'';$ischecked=false;  ?> /><img src="http://localhost:32185/themes/default/__RESOURCE__/recouse/images/icon-wein.png" /><?php echo $d['dname'];?></label></li>
 ...
 <ul class="payStyle">
 	 	...
 	 	<?php $ischecked=true;   if(is_array($payments)) { foreach($payments as $d) { ?>
  				         <?php if(empty($item['isverify'])||(!empty($item['isverify'])&&$d['online']==1)) { ?>
                                   <li><label><input name='payment'  type="radio"  value="<?php echo $d['code']?>" <?php echo $ischecked?'checked="checked"':'';$ischecked=false;  ?> /><img src="http://localhost:32185/themes/default/__RESOURCE__/recouse/images/icon-<?php echo $d['code']?>.png"  /><?php echo $d['name']?></label></li>
                                     <?php } ?>    
                        <?php } } ?>
...
<script src="http://localhost:32185/themes/default/__RESOURCE__/recouse/js/zepto.min.js" type="text/javascript"></script>
