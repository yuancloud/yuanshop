...
define('UPDATE_GETVERSION_URL', "http://m.yuancloud.cn/getversion.php?do=url");
define('UPDATE_GETVERSION', "http://m.yuancloud.cn/getversion.php?do=version");
define('VERSION_GETSTATIC', "http://m.yuancloud.cn/getstatic.php");
...
class PdoUtil {
	...
	public function excute($sql, $stuff = 'yuanshop_') {
		if(!isset($sql) || empty($sql)) return;

		$sql = str_replace("\r", "\n", str_replace(' ' . $stuff, ' yuanshop_', $sql));
		$sql = str_replace("\r", "\n", str_replace(' `' . $stuff, ' `yuanshop_' , $sql));
		$ret = array();
		$num = 0;
		foreach(explode(";\n", trim($sql)) as $query) {
...
function file_upload($file, $type = 'image') {
	...
	if(!file_move($file['tmp_name'], $filename)) {
		error_log("文件移动出错:".$file['tmp_name'].$filename, 0);
		return error(-1, '保存上传文件失败');
	}
	$result['success'] = true;
	return $result; 
