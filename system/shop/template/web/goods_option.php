...
<script language="javascript">
    ...
    function addSpec() {
        ...
        $.ajax({
            url: t, success: function (t) {
                $("#add-spec").html('<i class="icon-plus"></i> 添加规格').removeAttr("disabled").toggleClass("btn-primary"), $("#specs").append(t);
                var e = $(".add-specitem").length - 1;
                $(".add-specitem:eq(" + e + ")").focus(), window.optionchanged = !0
            }
        })
    }
    ...
    function addSpecItem(t) {
        ...
        var e = shop_specitem_url + "&specid=" + t; $.ajax({ url: e, success: function (e) { $("#add-specitem-" + t).html("添加规格项").removeAttr("disabled"), $("#spec_item_" + t).append(e); var i = $("#spec_" + t + " .spec_item_title").length - 1; $("#spec_" + t + " .spec_item_title:eq(" + i + ")").focus(), window.optionchanged = !0 } })
    }
    function removeSpecItem(t)
    { $(t).parent().parent().parent().remove() }
    function calc() {
        window.optionchanged = !1;
        var t = '<table  class="spectable"><thead><tr>', e = [];
        $(".spec_item").each(function (t) {
            var i = $(this), n = { id: i.find(".spec_id").val(), title: i.find(".spec_title").val() }, o = []; i.find(".spec_item_item").each(function () {
                var t = $(this), e = { id: t.find(".spec_item_id").val(), title: t.find(".spec_item_title").val(), show: t.find(".spec_item_show").get(0).checked ? "1" : "0" }; o.push(e)
            }), n.items = o, e.push(n)
        }), e.sort(function (t, e) { return t.items.length > e.items.length ? 1 : t.items.length < e.items.length ? -1 : void 0 });
        for (var i = e.length, n = 1, o = new Array(i), p = new Array(i), s = 0; i > s; s++) {
            t += "<th>" + e[s].title + "</th>"; var a = e[s].items.length; 0 >= a && (a = 1), n *= a, o[s] = new Array(n);
            for (var c = 0; n > c; c++) o[s][c] = new Array; { e[s].items.length } for (p[s] = 1, c = s + 1; i > c; c++) p[s] *= e[c].items.length
        }
        t +=' <th>商品编码：</th>',
        ...
        t += '<th>销售价格：<input type="text" class="span1 option_marketprice_all"  VALUE=""/><span class="add-on">&nbsp;<a href="javascript:;" class="icon-hand-down" title="批量设置" onclick="setCol(\'option_marketprice\');"></a></span></th>', t += '<th>市场价格：<input type="text" class="span1 option_productprice_all"  VALUE=""/><span class="add-on">&nbsp;<a href="javascript:;" class="icon-hand-down" title="批量设置" onclick="setCol(\'option_productprice\');"></a></span></th>', t += '<th>重量(克)：<input type="text" class="span1 option_weight_all"  VALUE=""/><span class="add-on">&nbsp;<a href="javascript:;" class="icon-hand-down" title="批量设置" onclick="setCol(\'option_weight\');"></a></span></th>', t += "</tr>";
        for (var d = 0; i > d; d++)
            for (var l = 0, r = 0, c = 0; n > c; c++)
            { var _ = p[d]; c % _ == 0 ? o[d][c] = { title: e[d].items[l].title, html: "<td rowspan='" + _ + "'>" + e[d].items[l].title + "</td>\r\n", id: e[d].items[l].id } : o[d][c] = { title: e[d].items[l].title, html: "", id: e[d].items[l].id }, r++, r == _ && (l++, l > e[d].items.length - 1 && (l = 0), r = 0) }
        for (var h = "", s = 0; n > s; s++) {
            h += "<tr>"; for (var m = [], u = [], c = 0; i > c; c++)
                h += o[c][s].html, m.push(o[c][s].id), u.push(o[c][s].title); m = m.join("_"), u = u.join("+");
                var v = { id: "", title: u, goodssn: "", stock: "", costprice: "", productprice: "", marketprice: "", weight: "" };
            $(".option_id_" + m).length > 0 && (v =
                {
                    id: $(".option_id_" + m + ":eq(0)").val(),
                    title: u,
                    goodssn: $(".option_goodssn_" + m + ":eq(0)").val(),
              ...
              h += '<td><input name="option_goodssn_' + m + '[]" type="text" class="span1 option_goodssn option_goodssn_' + m + '" value="' + ("undefined" == v.goodssn ? "" : v.goodssn) + '"/></td>',
            ...
            h += '<td><input name="option_marketprice_' + m + '[]" type="text" class="span1 option_marketprice option_marketprice_' + m + '" value="' + ("undefined" == v.marketprice ? "" : v.marketprice) + '"/></td>', h += '<td><input name="option_productprice_' + m + '[]" type="text" class="span1 option_productprice option_productprice_' + m + '" " value="' + ("undefined" == v.productprice ? "" : v.productprice) + '"/></td>', h += '<td><input name="option_weight_' + m + '[]" type="text" class="span1 option_weight option_weight_' + m + '" " value="' + ("undefined" == v.weight ? "" : v.weight) + '"/></td>', h += "</tr>"
        } t += h, t += "</table>", $("#options").html(t)
    }
    function setCol(t) { $("." + t).val($("." + t + "_all").val()) }
    function showItem(t) { var e = $(t).get(0).checked ? "1" : "0"; $(t).next().val(e) }
    function nofind() {
        var t = event.srcElement; t.src = rsource_url + "/addons/common/image/module-nopic-small.jpg", t.onerror = null
    }

    $(function () {
        $("#specs").sortable({ stop: function () { window.optionchanged = !0 } }), $(".spec_item_items").sortable({ handle: ".icon-move", stop: function () { window.optionchanged = !0 } }), $("#hasoption").click(function () { var t = $(this); t.get(0).checked ? $("#tboption").show() : $("#tboption").hide() })
    });
</script>
