<?php
        ...
        if ($operation == 'post') {
               ...
               $piclist = mysqld_selectall("SELECT * FROM " . table('shop_goods_piclist') . " where goodid=$id ORDER BY id ASC");
                ...
                if (count($options) > 0) {
                    ...
                    for ($i = 0; $i < $newlen; $i++) {
                        ...
                        $val = array("id" => "","title"=>"", "stock" => "", "costprice" => "", "goodssn" => "", "productprice" => "", "marketprice" => "", "weight" => "");
                        if(!empty($options))
                        {
                        ...
                        $hh .= '<td><input name="option_goodssn_' . $ids . '[]" type="text" class="span1 option_goodssn option_goodssn_' . $ids . '" value="' . $val['goodssn'] . '"/></td>';
                        $hh .= '<td>';
            ...
            if (checksubmit('submit')) {
                ...
                {
                    $filename =WEB_ROOT.'/sku_image/'.$_GP['goodssn'].'.jpg';
                    $filename_png =WEB_ROOT.'/sku_image/'.$_GP['goodssn'].'.png';
                    $filename_jpeg =WEB_ROOT.'/sku_image/'.$_GP['goodssn'].'.jpeg';
                    if (file_exists($filename))
                    {
                        $data['thumb'] = '../sku_image/'.$_GP['goodssn'].'.jpg';
                    }
                    elseif (file_exists($filename_png))
                    {
                        $data['thumb'] = '../sku_image/'.$_GP['goodssn'].'.png';
                    }
                    elseif (file_exists($filename_jpeg))
                    {
                        $data['thumb'] = '../sku_image/' . $_GP['goodssn'].'.jpeg';
                    }
                }
                if (empty($id)) {
  ...
  $tspecitems = mysqld_select("SELECT id FROM " . table('shop_goods_spec_item') . " WHERE id = :id", array(':id' => $get_item_id));
                ...
                $option_goodssn = $_POST['option_goodssn'];
                ...
                for ($k = 0; $k < $len; $k++) {
                    ...
                    $a = array(
                        ...
                        "goodssn" => $_GP['option_goodssn_' . $ids][0],   
                    ...
                    if (!empty($_GP['option_goodssn_' . $ids][0])&& $_GP['option_stock_' . $ids][0]>0) {
                        $filename = WEB_ROOT . '/sku_image/' . $_GP['option_goodssn_' . $ids][0] . '.jpg';
                        $filename_png = WEB_ROOT . '/sku_image/' . $_GP['option_goodssn_' . $ids][0] . '.png';
                        $filename_jpeg=WEB_ROOT . '/sku_image/' . $_GP['option_goodssn_' . $ids][0] . '.jpeg';
                        $imgurl = "";
                        if (file_exists($filename)) {
                            $imgurl = '../sku_image/' . $_GP['option_goodssn_' . $ids][0] . '.jpg';
                        } elseif (file_exists($filename_png)) {
                            $imgurl = '../sku_image/' . $_GP['option_goodssn_' . $ids][0] . '.png';
                        }elseif (file_exists($filename_jpeg))
                        {
                            $imgurl = '../sku_image/' . $_GP['option_goodssn_' . $ids][0] . '.jpeg';
                        }
                        $attachimage[] = $imgurl;
                    }
                    $totalstocks+=$a['stock'];
                    //if(empty($a['goodssn']))$a['goodssn'] = 
                ...
                if (count($attachimage)>0)
                {
                    mysqld_delete('shop_goods_piclist', array('goodid' => $id));
                    foreach ($attachimage as $row) {
                        if (!empty($row))
                        {
                        $data = array(
                            'goodid' => $id,
            ...
            if (!empty($_GP['cate_2'])) {
                ...
                $condition .= " AND goods.ccate = '{$cid}'";
            } elseif (!empty($_GP['cate_1'])) {
                $cid = intval($_GP['cate_1']);
                $condition .= " AND goods.pcate = '{$cid}'";
            }

            if (isset($_GP['status'])) {
                $condition .= " AND goods.status = '" . intval($_GP['status']) . "'";
            }
            $list = mysqld_selectall("SELECT DISTINCT goods.* FROM " . table('shop_goods') . " goods left join ".table('shop_goods_option')." goods_options on goods_options.goodsid=goods.id WHERE  goods.deleted=0 $condition ORDER BY goods.status DESC, goods.displayorder DESC, goods.id DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize);
            $total=mysqld_selectcolumn("SELECT COUNT(DISTINCT goods.id)  FROM " . table('shop_goods') . " goods left join ".table('shop_goods_option')." goods_options on goods_options.goodsid=goods.id WHERE  goods.deleted=0 $condition ORDER BY goods.status DESC, goods.displayorder DESC, goods.id DESC");
            #$total = mysqld_selectcolumn('SELECT COUNT(DISTINCT goods.*) FROM ' . table('shop_goods') ." goods left join ".table('shop_goods_option'). " goods_options on goods_options.goodsid=goods.id WHERE goods.deleted=0 $condition");
