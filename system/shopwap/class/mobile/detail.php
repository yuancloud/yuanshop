<?php
       ...
       $goods_piclist_count = mysqld_selectcolumn("SELECT count(*) FROM " . table('shop_goods_piclist') . " WHERE goodid = :goodid", array(':goodid' => $goodsid));
        ...
        //规格及规格项
           ...
           //处理规格项
           $options = mysqld_selectall("select id,title,thumb,marketprice,goodssn,productprice, stock,weight,specs from " . table('shop_goods_option') . " where goodsid=:id order by id asc", array(':id' => $goodsid));
